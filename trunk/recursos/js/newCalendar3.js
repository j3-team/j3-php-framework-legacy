function Epoch(name,mode,targetelement,imgElement,multiselect,showweeks,weekend,datemin,datemax,format)
{
	if (!format)
		format = "d/m/Y";
	...
	this.minyear = parseInt(datemin.substring(6,10),10);
	this.maxyear = parseInt(datemax.substring(6,10),10);
	this.minMonth = parseInt(datemin.substring(3,5),10)-1;
	this.maxMonth = parseInt(datemax.substring(3,5),10)-1;
	this.minDay = parseInt(datemin.substring(0,2),10);
	this.maxDay = parseInt(datemax.substring(0,2),10);
	...
	if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
	{
		...
		/** IFRAME */
		this.iframe.style.top = this.getTop(targetelement) + this.topOffset + 'px';
		this.iframe.style.left = this.getLeft(targetelement) + this.leftOffset + 'px';
		this.iframe.style.width = "185px";
		this.iframe.style.height = "175px";
		document.body.appendChild(this.iframe);
		
		document.body.appendChild(this.calendar);
		this.tgt.calendar = this;
		//this.tgt.onfocus = function () {this.calendar.show();}; //the calendar will popup when the input element is focused
		this.img.calendar = this;
		this.img.style.cursor='pointer';
		this.img.onclick = function () {
			//update position
			this.calendar.calendar.style.top = this.calendar.getTop(targetelement) + targetelement.offsetHeight + 'px';
			this.calendar.calendar.style.left = this.calendar.getLeft(targetelement) + this.calendar.leftOffset + 'px';
			this.calendar.iframe.style.top = this.calendar.getTop(targetelement) + targetelement.offsetHeight + 'px';
			this.calendar.iframe.style.left = this.calendar.getLeft(targetelement) + this.calendar.leftOffset + 'px';
...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.rangeYearLower = this.minyear;
	this.rangeYearUpper = this.maxyear;
	...
	this.showWeeks = this.showweeks; //whether the week numbers will be shown
...
Epoch.prototype.setLang = function()  //all language settings for Epoch are made here.  Check Date.dateFormat() for the Date object's language settings
{
	...
	this.monthup_title = 'Ir a mes siguiente';
	this.monthdn_title = 'ir a mes anterior';
	this.clearbtn_caption = ' Limpiar ';
	this.closebtn_caption = ' Cerrar ';
	this.clearbtn_title = 'Limpiar cualquier fecha seleccionada';
	this.maxrange_caption = 'RANGO NO PERMITIDO.\n\n' 
							+ 'Mínimo:\t' + this.months_sh[this.minMonth] + ', ' + this.rangeYearLower + '.\n'
							+ 'Máximo:\t' + this.months_sh[this.maxMonth] + ', ' + this.rangeYearUpper + '.';
};
//-----------------------------------------------------------------------------
...
Epoch.prototype.createMainHeading = function () //PRIVATE: Creates the primary calendar heading, with months & years
{
	...
	this.monthSelect.onchange = function() {
		...
		this.owner.goToMonth(this.owner.displayYear,this.owner.displayMonth,0);
//		this.owner.validMonth();
	};

	var aa;
	
	this.yearSelect.onclick = function() {
		aa = this.value;
	}
	this.yearSelect.onchange = function() {
		this.owner.displayMonth = this.owner.monthSelect.value;
		this.owner.displayYear = this.value; 
		this.owner.goToMonth(this.owner.displayYear,this.owner.displayMonth,aa);
		this.owner.validMonth();
	};
...
Epoch.prototype.createFooter = function () //PRIVATE: creates the footer of the calendar - goes under the calendar cells
{
	...
	btnClose.setAttribute('value',this.closebtn_caption);
	btnClose.owner = this;
	btnClose.onclick = function() { this.owner.hide();};
...
Epoch.prototype.resetSelections = function (returnToDefaultMonth)  //PRIVATE: reset the calendar's selection variables to defaults
{
	...
	if(returnToDefaultMonth == true) {
		this.goToMonth(this.displayYearInitial,this.displayMonthInitial,0);
	}
	else {
		this.reDraw();
	}
...
//-----------------------------------------------------------------------------
Epoch.prototype.goToMonth = function (year,month,y) //PUBLIC: sets the calendar to display the requested month/year
{
	if (((year < this.rangeYearUpper) || (year == this.rangeYearUpper && month <= this.maxMonth)) &&
		((year > this.rangeYearLower) || (year == this.rangeYearLower && month >= this.minMonth))) {
		...
		if (y!=0)
			this.yearSelect.value = this.displayYear = y;
	}
};
//-----------------------------------------------------------------------------
Epoch.prototype.validMonth = function () //PUBLIC: valida los rangos para los meses
{
	var aa = 0;
	var opt;
	...
	while(this.monthSelect.options.length>0) {
		this.monthSelect.removeChild( this.monthSelect.options[0] );
	}
	...
	for(aa=0;aa<12;aa++)
	{
		...
		if(this.displayMonth == aa) {
			opt.setAttribute('selected','selected');
		}
		opt.appendChild(document.createTextNode(this.months_sh[aa]));
		this.monthSelect.appendChild(opt);
	}
	...
	{
		for (aa = 0; aa < this.minMonth; aa++)
		{
//			alert(this.monthSelect.options[0].text);
			this.monthSelect.removeChild( this.monthSelect.options[0] );
		}
	}
	...
	{
		for (aa = 11; aa > this.maxMonth; aa--)
		{
			this.monthSelect.removeChild( this.monthSelect.options[ this.monthSelect.options.length-1 ] );
		}
	}
...
Epoch.prototype.nextMonth = function () //PUBLIC: go to the next month.  if the month is december, go to january of the next year
{
	...
	if (this.monthSelect.value < this.maxMonth && this.yearSelect.value == this.rangeYearUpper)
	{
		this.monthSelect.value++;
	} else if (this.yearSelect.value < this.rangeYearUpper)
	{
...
{
	//increment the month/year values, provided they're within the min/max ranges
	//alert (this.monthSelect.value + "--" + this.minMonth);
	//alert (this.yearSelect.value + "--" + this.rangeYearLower);
	if (this.monthSelect.value > this.minMonth && this.yearSelect.value == this.rangeYearLower) {
		this.monthSelect.value--;
	} else if (this.yearSelect.value > this.rangeYearLower) {
...
CalHeading.prototype.onclick = function ()
{
	...
	for(var i=0;i<cells.length;i++) //cycle through all the cells in the calendar, selecting all cells with the same dayOfWeek as this heading
	{
		if(cells[i].dayOfWeek == this.headObj.dayOfWeek && (!owner.selCurMonthOnly || cells[i].date.getMonth() == owner.displayMonth && cells[i].date.getFullYear() == owner.displayYear)) //if the cell's DoW matches, with other conditions
		{
			...
			{
				if(arrayIndex(owner.selectedDates,cells[i].date) == -1) { //if the date isn't already in the array
...
CalCell.prototype.onmouseover = function () //replicate CSS :hover effect for non-supporting browsers <iehack>
{
	...
	if ( this.className == 'wkend' || this.className == 'wkday' || this.className == 'wkday curdate' ){
		//this.setAttribute('class', ' allowday');
		//this.setAttribute('className', ' allowday');
		this.className = "allowday";
	}
};
//-----------------------------------------------------------------------------
...
CalCell.prototype.onclick = function () 
{
	...
	//alert(this.className);
	if ( this.className == 'allowday' || this.className == 'wkday' || this.className == 'wkday curdate' ) {
		if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
		{
			if(owner.selectMultiple == true)  //if we can select multiple cells simultaneously, add the currently selected cell's date to the selectedDates array
			{
				...
				{
					if(arrayIndex(owner.selectedDates,cell.date) == -1) {
			...
			else //if we can only select one cell at a time
			{
				...
				{
					owner.tgt.value = owner.selectedDates[0].dateFormat(owner.dateFormat);
					if(owner.mode == 'popup') {
						owner.tgt.focus();
						owner.hide();
					}
...
CalCell.prototype.setClass = function ()  //private: sets the CSS class of the cell based on the specified criteria
{
	if(this.selected) {
		...
		if (this.date.getDay() > 0 && this.date.getDay() < 6) {
			...
			this.cellClass = 'wkend3';
		}
	}
	else  {
		if ((this.owner.rangeYearLower == this.owner.yearSelect.value && this.owner.displayMonth == this.owner.minMonth && this.date.getDate() < this.owner.minDay)
		 || (this.owner.rangeYearUpper == this.owner.yearSelect.value && this.owner.displayMonth == this.owner.maxMonth && this.date.getDate() > this.owner.maxDay)) {
		 	if (this.date.getDay() > 0 && this.date.getDay() < 6) {
			 	this.cellClass = 'notmnth';	
			} else {
				this.cellClass = 'wkend3';
			...
			if (this.date.getDay() > 0 && this.date.getDay() < 6) {
				...
				if (this.owner.weekEnd)
					this.cellClass = 'wkend';
				else
					this.cellClass = 'wkend2';
			}
		}		
	}
...
/*****************************************************************************/
function arrayIndex(arreglo,searchVal,startIndex) //similar to array.indexOf() - created to fix IE deficiencies
{
	startIndex = (startIndex != null ? startIndex : 0); //default startIndex to 0, if not set
	for(var i=startIndex;i<arreglo.length;i++)
	{
		if(searchVal == arreglo[i]) {
			return i;
		}
	}
	return -1;
