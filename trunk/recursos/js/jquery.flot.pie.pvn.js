...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        var options = {
            ...
            colors: ["#2c5885", "#448899", "#ffcc44", "#d56731", "#bbddbb", "#d2e1f0", "#aa7788", "#ffffff", "#ffccbb", "#ffee77" , "#bbaa88" , "#eeddbb", "#1c3855" ],
            legend: {
                show: true,
			...
			pie: {
				...
				labelOffsetFactor: 1, //as part of radius default = 5/6
  ...
  /*              }
    ...
    */            res.push(s);
        ...
        function setupGrid() {
			...
			if (options.pie.show) {
				pieChartRadius = options.pie.pieChartRadius || Math.min(canvasWidth,canvasHeight-4)/2;
        ...
        function draw() {
			...
			var lefts = leftTop.reverse().concat(leftBottom.reverse());
			var rights = rightTop.concat(rightBottom);
			drawMyLabel(lefts, "L");
			drawMyLabel(rights, "R");
			
        }
		
		
		function drawMyLabel(arr, what) {
			var marginX = 0;
			if (what == "R") { // Right
				marginX = pieCenterLeft + pieChartRadius + 10;
			} else {		   // Left
				marginX = pieCenterLeft - pieChartRadius - 26 - 12;
			}
			
			var middle = Math.floor(arr.length / 2);
			var marginY = 0;			
			if (arr.length % 2 == 0) {
				marginY = pieCenterTop - ((middle * 17) + ((middle - 1) * 6) + 3);
			} else {
				marginY = pieCenterTop - ((middle * (17+6)) + 9);
			}
		
			for (var i=0; i<arr.length; i++) {
				printMyLabel(arr[i], marginX, marginY, what);
				marginY += 25;
			}
		}
		
		function printMyLabel(mylabel, posX, posY, what) {
			var html = '<span  class="pieLabel" id="pieLabel2'+pieArcIndex+'" style="position:absolute;top:' + (posY + 2) + 'px;left:' + (posX + 2) + 'px;"><div style="border: 1px solid '+mylabel.color+';">&nbsp;</div></span>';
			var html2 = '<span  class="pieLabel" id="pieLabel'+pieArcIndex+'" style="position:absolute;top:' + (posY) + 'px;left:' + (posX) + 'px;"><div style="border: 1px solid '+mylabel.color+';">' + mylabel.text + '</div></span>';
			
			target.append(html);
			target.append(html2); //pieCenterTop
			
			if (what == "L") { // Left
				target.append(drawLine(posX+27, posY+7, posX+42, mylabel.posY, mylabel.color));
				target.append(drawLine(posX+42, mylabel.posY, mylabel.posX, mylabel.posY, mylabel.color));
			} else {		   // Right
				target.append(drawLine(mylabel.posX, mylabel.posY, posX-15, mylabel.posY, mylabel.color));
				target.append(drawLine(posX-15, mylabel.posY, posX, posY+7, mylabel.color));
			}
		}
        ...
        function drawShadow() {
			...
			var arcEndAngle = Math.PI * (- 0.5 + 2 * (pieIndex + 1));		

			ctx.beginPath();
			ctx.lineWidth = 0;
			ctx.moveTo(pieCenterLeft+3,pieCenterTop+3); // Center of the pie
		...
		//PIE: draw series pie
        function drawSeriesPie(serie, error){
			var error = 0;
			...
			var arcStartAngle = Math.PI * (- 0.5 + 2 * pieIndex); // -0.5 sets set the start to be top
			var arcEndAngle = Math.PI * (- 0.5 + 2 * (pieIndex + thisvalue))-error;
			ctx.beginPath();
			ctx.lineWidth = 0;
			...
			if (options.pie.showLabel && typeof(options.pie.labelFormatter) == 'function') {
				...
				var xh = pieCenterLeft + Math.round(Math.cos(halfAngle) * lo); //usados para el inicio de la rayita
				var yh = pieCenterTop + Math.round(Math.sin(halfAngle) * lo);

				
				if (xh > pieCenterLeft) {
					if (yh <= pieCenterTop) { // Top - Right
						rightTop.push(new MyLabel( xh, yh, text, serie.color ));
					} else {                  // Bottom - Right
						rightBottom.push(new MyLabel( xh, yh, text, serie.color ));
					}
				} else {
					if (yh <= pieCenterTop) { // Top - Left
						leftTop.push(new MyLabel( xh, yh, text, serie.color ));
					} else {                  // Bottom - Left
						leftBottom.push(new MyLabel( xh, yh, text, serie.color ));
					}
				}
				...
				//var html = '<span  class="pieLabel" id="pieLabel'+pieArcIndex+'" style="position:absolute;top:' + (plotOffset.top + yh) + 'px;left:' + (plotOffset.left + xh) + 'px;"><div>' + text + "</div></span>";
					...
					//var html = '<span  class="pieLabel" id="pieLabel2'+pieArcIndex+'" style="position:absolute;top:' + (plotOffset.top + yh + 2) + 'px;left:' + (plotOffset.left + xh + 2) + 'px;"><div style="border: 1px solid '+serie.color+';">&nbsp;</div></span>';
					//var html2 = '<span  class="pieLabel" id="pieLabel'+pieArcIndex+'" style="position:absolute;top:' + (plotOffset.top + yh) + 'px;left:' + (plotOffset.left + xh) + 'px;"><div style="border: 1px solid '+serie.color+';">' + text + '</div></span>';
					//var html = '<span  class="pieLabel" id="pieLabel2'+pieArcIndex+'" style="position:absolute;top:' + (plotOffset.top + yh + 2) + 'px;left:' + (marginLeft + 2) + 'px;"><div style="border: 1px solid '+serie.color+';">&nbsp;</div></span>';
					//var html2 = '<span  class="pieLabel" id="pieLabel'+pieArcIndex+'" style="position:absolute;top:' + (plotOffset.top + yh) + 'px;left:' + (marginLeft) + 'px;"><div style="border: 1px solid '+serie.color+';">' + text + '</div></span>';
...
function drawLine( Ax, Ay, Bx, By, color )
{
	...
	{
		line += "<div style='position:absolute;left:"+ Math.round( Ax+(Bx-Ax)*i/lineLength  ) +"px;top:"+ Math.round( Ay+(By-Ay)*i/lineLength  ) +"px;width:1px;height:1px;font-size:1px;line-height:0;background:"+color+"'>&nbsp;</div>";
	}
...
/**
	 Clase para almacenar los labels
*/
function MyLabel(posX, posY, text, color) {
	this.posX = posX;
	this.posY = posY;
	this.text = text;
	this.color = color;
...
if(!jQueryFlot) var jQueryFlot = {};

jQueryFlot.addValue = function(arrValues, lab, val) {
	arrValues.push({data: val, label: lab });
}

jQueryFlot.showGraph = function (idGraph, idLegend, arrValues, numColumns, colores) {
	document.getElementById(idLegend).innerHTML = "";

	jQuery(function () {
		jQuery.plot(jQuery("#"+idGraph), 
			arrValues, {
			pie: {
				show: true,
				showLabel: true,
